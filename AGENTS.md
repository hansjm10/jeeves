...
- safe-shell-search: Enforce pruner-first codebase discovery and evidence-grounded claims. Use when: (1) searching across file contents to find implementations, (2) reading known file paths for inspection, (3) building evidence for behavior claims, (4) any codebase investigation that would otherwise use shell grep/cat/find. Triggers on: code search, file discovery, evidence gathering, investigation loop, codebase exploration. (file: skills/common/safe-shell-search/SKILL.md)
- jeeves-task-spec-check: Jeeves-specific task spec-check adapter defining MCP state contracts, artifact schemas, and PASS/FAIL handling. Use when: (1) verifying task acceptance criteria in the task_spec_check phase, (2) producing phase-report.json or task-feedback.md artifacts, (3) enforcing filesAllowed compliance, (4) writing criterion-level evidence with structured verdicts. Triggers on: spec check, task verification, acceptance criteria check, phase report, task feedback. (file: skills/implement/jeeves-task-spec-check/SKILL.md)
