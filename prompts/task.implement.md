...
<constraints> IMPORTANT: This phase allows code changes, but is **strictly scoped**.
...
Call `state_get_tasks`, locate the task with ID currentTaskId, and record:
...
- Record what you changed and why with `state_append_progress`
- Update issue control with `state_update_issue_control` to request a fresh-context restart of this phase by setting:
...
If you cannot safely clean/stash the unexpected files → STOP and record the blocker with `state_append_progress`.
...
Record the blocker with `state_append_progress`.
...
Call `state_set_task_status` with the current task ID and status `"in_progress"`.
