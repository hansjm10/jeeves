...
4. Read structured memory first, then progress fallback

   Call `state_get_memory` and load:
   - `working_set` entries (active work context)
   - `decisions` entries (durable implementation constraints)
